#!/usr/bin/env bash
#
# Deploy KennelFlow Boarding to WordPress.org SVN (trunk, tags/VERSION, assets/).
#
# Usage (from this directory):
#   VERSION=0.1.2 ./deploy-to-wordpress-org.sh
#   VERSION=0.1.2 ./deploy-to-wordpress-org.sh --commit
#
set -euo pipefail
...
SVN_DIR="${SVN_DIR:-$(dirname "$PLUGIN_ROOT")/../kennelflow-boarding-wporg-svn}"
VERSION="${VERSION:-$(grep -E "^ \* Version:" "$PLUGIN_ROOT/kennelflow-boarding.php" | sed 's/[^0-9.]//g')}"
...
if [[ -z "$VERSION" ]]; then
	echo "Could not detect VERSION from kennelflow-boarding.php" >&2
	exit 1
fi
...
rsync -a --delete \
	...
	--exclude='/vite.facility.config.js' \
	--exclude='/vite.config.js' \
	--exclude='/.wordpress-org/' \
	--exclude='*.zip' \
	--exclude='*.sh' \
	--exclude='*.md' \
	--exclude='apply-rename.php' \
...
if [[ -d "$ASSETS_SRC" ]]; then
	...
	rsync -a \
		--exclude='README.txt' \
		--exclude='.DS_Store' \
		--exclude='KennefFlowBoarding.png' \
